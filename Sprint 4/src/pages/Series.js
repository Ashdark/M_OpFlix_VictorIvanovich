      ...
      <SafeAreaView style={styles.background}>
        {(this.state.search !== null) & (this.state.search !== "") & (this.state.searchDate == "" || null) ?
          <View style = {styles.lista}>
            <FlatList
              data={this.state.Series.filter(element => element.idTipo === 2).joinWith(this.state.Categorias,'idCat').filter(element => element.nomeCat.toLowerCase() == this.state.search.toLowerCase())}
        ...
        ((this.state.searchDate !== null) & (this.state.searchDate !== "") & (this.state.search == "" || null)) ?
          <View style = {styles.lista}>
            <FlatList
              data={this.state.Series.filter(element => element.idTipo == 2).joinWith(this.state.Categorias,'idCat').filter(element => element.dataLancamento == this.state.searchDate)}
              keyExtractor={item => item.idSf}
              ItemSeparatorComponent={
                () => <View style={{ height: 10,width: Dimensions.get('window').width, backgroundColor: 'red' }}/>
            }
          ...
          <View style = {styles.lista}>
            <FlatList
              data={this.state.Series.filter(element => element.idTipo == 2).joinWith(this.state.Categorias,'idCat').filter(element => element.dataLancamento == this.state.searchDate & element.nomeCat.toLowerCase() == this.state.search.toLowerCase())}
              keyExtractor={item => item.idSf}
              ItemSeparatorComponent={
                () => <View style={{ height: 10,width: Dimensions.get('window').width, backgroundColor: 'red' }}/>
            }
          ...
          <View style = {styles.lista}>
          <FlatList
            data={this.state.Series.filter(element => element.idTipo == 2).joinWith(this.state.Categorias,'idCat').filter(element => element.titulo != null)}
            keyExtractor={item => item.idSf}
            ItemSeparatorComponent={
              () => <View style={{ height: 10,width: Dimensions.get('window').width, backgroundColor: 'red' }}/>
          }
